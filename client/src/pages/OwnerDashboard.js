  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.tabs}>
        ...
        >🚗 Mere Vehicles ({vehicles.length})</button>
        <button
          ...
          <div>
                ...
                <form onSubmit={handleAddVehicle}>
                  <div style={styles.formGrid}>
                    ...
                    <div style={{...styles.inputGroup, gridColumn: '1/-1'}}>
                      ...
                      <textarea style={{...styles.input, height: '80px'}} placeholder="Vehicle ke baare mein likhо"
            ...
            {loading ? <p>Loading... ⏳</p> : (
              <div style={styles.grid}>
                {vehicles.length === 0 ? (
                  <p style={styles.noData}>Koi vehicle nahi hai — pehla vehicle add karo! 🚗</p>
                ) : vehicles.map((v) => (
                ...
                <div style={styles.bookingHeader}>
                  ...
                  <span style={{
                    ...
                    backgroundColor: b.status === 'confirmed' ? '#dcfce7' :
                      b.status === 'pending' ? '#fef9c3' :
                      b.status === 'cancelled' ? '#fee2e2' : '#e0f2fe',
                    color: b.status === 'confirmed' ? 'green' :
                      b.status === 'pending' ? '#b45309' :
                      b.status === 'cancelled' ? 'red' : 'blue'
                  }}>{b.status.toUpperCase()}</span>
                </div>
                <p style={styles.info}>👤 Client: {b.client?.name} | 📞 {b.client?.phone}</p>
                <p style={styles.info}>📅 {new Date(b.startDate).toLocaleDateString()} → {new Date(b.endDate).toLocaleDateString()}</p>
                <p style={styles.info}>💰 Total: ₹{b.totalAmount} | Advance: ₹{b.advanceAmount}</p>
                <p style={styles.info}>📍 Pickup: {b.pickupLocation}</p>
                {b.status === 'pending' && (
...
const styles = {
  ...
  logoutBtn: {
    ...
    color: '#f97316', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600'
  },
  ...
  submitBtn: {
    marginTop: '16px', padding: '12px 32px', backgroundColor: '#16a34a',
  ...
  deleteBtn: {
    marginTop: '12px', padding: '8px 16px', backgroundColor: '#fee2e2',
    color: 'red', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600'
  },
  bookingCard: {
  ...
  confirmBtn: {
    padding: '8px 20px', backgroundColor: '#dcfce7', color: 'green',
    border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600'
  },
  cancelBtn: {
    padding: '8px 20px', backgroundColor: '#fee2e2', color: 'red',
    border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600'
  },
